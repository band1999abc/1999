...
(function () {
    ...
    const paginationEl = document.getElementById('live-pagination');
    if (!listEl) return;
    const PAGE_SIZE = 10;
    let currentPage = 1;
    let orderedLives = [];
    ...
    function orderUpcoming(lives) {
        return lives.slice().sort(compareManualOrder);
    }
    ...
    function renderPage() {
        const pageCount = Math.max(1, Math.ceil(orderedLives.length / PAGE_SIZE));
        currentPage = Math.min(currentPage, pageCount);
        const start = (currentPage - 1) * PAGE_SIZE;
        const visible = orderedLives.slice(start, start + PAGE_SIZE);
        ...
        visible.forEach(function (item, index) {
            if (item.section === 'past' && !archiveShown) {
            ...
            if (index > 0 && item.section === 'upcoming') {
                const divider = document.createElement('div');
                divider.className = 'live-divider';
                listEl.appendChild(divider);
            }

            listEl.appendChild(item.section === 'upcoming'
                ? renderEntry(item.live, item.live.id === nextLiveId)
                : renderPastEntry(item.live));
        });
    ...
    async function load() {
        try {
            ...
            const upcoming = orderUpcoming(all.filter(l => l.date >= today));
            ...
            nextLiveId = upcoming.length > 0 ? upcoming[0].id : null;
            orderedLives = upcoming
                .map(live => ({ section: 'upcoming', live }))
                .concat(past.map(live => ({ section: 'past', live })));
            renderPage();
